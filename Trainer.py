...
class Trainer(object):
    ...
    def __init__(self, initial_features_dims, device, datamanager, best_model_file):
        ...
        self.train_data = datamanager.get_dataset_per_batch(this_batch_size=3, this_number_of_batchs=6)
        self.train_data = datamanager.get_epoch_data(self.train_data)

        self.val_data = datamanager.get_dataset_per_batch(this_batch_size=3, this_number_of_batchs=6)
        self.val_data = datamanager.get_epoch_data(self.val_data)

        self.test_data = datamanager.get_dataset_per_batch(this_batch_size=3, this_number_of_batchs=6)
    ...
    def fit(self):
        ...
        for i in range(1, self.epochs + 1):
            ...
            print("Epoch {i}/{self.epochs}:\t MSE loss = {loss_vag} @ {datetime.now()}")
    ...
    def train_one_epoch(self, model, optimizer, epoch_data, device):
        ...
        for val in permutation:
            ...
            features1, adj1, features2, adj2, nodes1, nodes2, pred = sample
        ...
        with torch.no_grad():
            ...
            for cur_data in eval_epoch_data:
                ...
                if len(batch_output) == len(y): 
                    ...
                    print("Un gros probleme !!")
                total_pred += len(batch_output)
            print('Good preds: ', good_pred)
